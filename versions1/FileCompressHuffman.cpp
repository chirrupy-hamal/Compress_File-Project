...
void FileCompressHuffMan::CompressFile(const string &strFilePath)
{
	...
	char *pReadBuff = new char[1024];//每次读1K的数据
	/*
...
void FileCompressHuffMan::UNCompressFile(const std::string &strFilePath)
{
	...
	for (size_t i = 0; i < lineCount; ++i)
	{
		...
		_charInfo[strContent[0]]._charCount = atoi(strContent.c_str() + 2);//+2是因为前面有字符和逗号
	}
	...
	FILE *fOut = fopen(strUNComFile.c_str(), "w");
	assert(fOut);
